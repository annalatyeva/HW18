...
function makeFive() {
	window.localStorage.setItem('srring', null);
...
function makeSeven() {
	window.localStorage.setItem('objectSeven', JSON.stringify(obj2));
	console.log('Объект obj был записан в Local Storage.');
}
...
function makeEight() {
	window.localStorage.setItem('objectEight', JSON.stringify(obj3));
	console.log('Объект obj был записан в Local Storage.');
}
...
function makeTen() {
	window.localStorage.setItem('arrayTen', JSON.stringify(arr2));
	console.log('Массив arr был записан в Local Storage.');
}
...
function makeEleven() {
	window.localStorage.setItem('arrayEleven', JSON.stringify(arr3));
	console.log('Массив arr был записан в Local Storage.');
}
...
function showEleven() {
	arrayEleven = JSON.parse(window.localStorage.getItem('arrayEleven'));
...
const showNine = () => {
	...
	if (arrayNineString) {
		const arrayNineParse = JSON.parse(arrayNineString);
		const result = document.querySelector('.practicum14');
		result.textContent ='';
		for (const key in arrayNineParse) {
			result.innerHTML += `${key} ${arrayNineParse[key]} <br>
		`}
	} else {
...
const showEight = () => {
	const objectEightParse = JSON.parse(window.localStorage.getItem('objectEight'));
	let result15 = document.querySelector('.practicum15');
	result15.innerText = '';
	if (objectEightParse) {
		for (const key in objectEightParse) {
			result15.innerHTML += `${objectEightParse[key]};<br>`
			}
	}
	else {
		result15.innerText = 'Объект objectEight не найден в Local Storage';
	}
};
...
const showSeven = () => {
	const objectSevenParse = JSON.parse(window.localStorage.getItem('objectSeven'));
	let result16 = document.querySelector
	('.practicum16');
	result16.innerText = '';
	if (objectSevenParse) {
		result16.innerText = `В ${objectSevenParse.city}e живёт ${objectSevenParse.population} человек.`;
	}
	else {
		result16.innerText = 'Объект objectSeven не найден в Local Storage';
	}
};
...
const showSix = () => {
	const objectSixParse = JSON.parse(window.localStorage.getItem('objectSix'));
	result17 = document.querySelector('.practicum17');
	result17.innerText = '';
	if (objectSixParse) {
		result17.innerHTML += `Привет! Я  ${objectSixParse.name}. Мне ${objectSixParse.age} лет.<br>`
	}
};
...
const addToNumbers = () => {
	...
	nambersString = JSON.stringify(numbers);
	window.localStorage.setItem ('task19', `${nambersString}`);
	numberCount++;
};
...
const removeLastElement = () => {
	numbers = JSON.parse(window.localStorage.getItem('task19'));
	console.log(numbers);
	if (numbers.length > 0) {
		numbers.pop ();
		nambersString = JSON.stringify(numbers);
...
const removeFromCart = () => {
	...
	if (cartItems && cartItems != '[]' && cartItems != '') {
		...
		console.log ('Массив cartItems пуст или не существует');
	}
};
...
const showCart = () => {
	cartItems = JSON.parse(window.localStorage.getItem('cartItems'));
	let practicum23 = document.querySelector('.practicum23');
	practicum23.innerHTML = '<h5>В корзине:<br></h5>';
	if (cartItems && cartItems != '[]' && cartItems != '') {
		for (let i = 0; i < cartItems.length; i++) {
			practicum23.innerHTML += `${cartItems[i]} <br>`;
		}
	}
	else {
		practicum23.innerHTML += 'ничего нет';
	}
};
...
const updateCartCount = () => {
	...
	if (cartItems && cartItems != '[]' && cartItems != '') {
		practicum24.innerText = 'Количество товаров в корзине ' + cartItems.length;
	}
	else {
		practicum24.innerText = 'Количество товаров в корзине 0';
	}
};
...
const checkCookie = () => {
	const cookieContainer2 = document.querySelector('.practicum28');
	const username = getCookie('username', false);
	if (username !== '') {
...
const setCookie = (name, value) => {
	...
	console.log (`Куки с именем ${name} и значением ${value} установлен.`);
};
...
const deleteCookie = (name) => {
	...
	console.log (`Куки с именем ${name} удалён.`)
};
