...
let superheroCard;
let i;
superheros.forEach(superheroe => {
  i = superheros.indexOf(superheroe) + 1;
  superheroCard = document.createElement('article');
  superheroCard.className = 'superhero-card';
  superheroCard.innerHTML = `<h2>${superheroe.name}</h2>
  <div class='superhero-biography'><p>Вселенная:</p><p>${superheroe.universe}</p></div>
  <div class='superhero-biography'><p>Альтер Эго:</p><p>${superheroe.alterego}</p></div> 
  <div class='superhero-biography'><p>Род деятельности:</p><p>${superheroe.occupation}</p></div>
  <div class='superhero-biography'><p>Друзья:</p><p>${superheroe.friends}</p></div>
  <div class='superhero-biography'><p>Суперсилы:</p><p>${superheroe.superpowers}</p></div>
  <img src=${superheroe.url} alt="superhero image">
  <form id="rating-area${i}">
  <input
    type="radio"
    id="star5-${i}"
    name="rating${i}"
    onclick="checkRating(this.name, this.id)"
    value="5"/>
  <label
    for="star5-${i}"
  ...
  document.querySelector('.surerheros-container').appendChild(superheroCard);

  let memoryRating = window.localStorage.getItem(`rating${i}`);
  if (memoryRating) {
    document.getElementById(memoryRating).checked = true;
  }
});

function checkRating (name, id) {
